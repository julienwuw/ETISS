...
/**
    @attention this function relies on spaces being an acceptable fill character in case of field/input length missmatch
*/
class form_string
{
private:
...
public:
    ...
    form_string(T... str)
    {
        ...
        for (size_t i = 0;i<sizeof...(str);i++){
            if (placeholder[i]){ // placeholder field
...
#endif
            T tmp = value / 10;
            char c = (char) (value % 10); // the compiler should detect the division and use the remainder here without calculating modulo
            value = tmp;
            buffer[pos] = '0' + c;
            pos--;
        } while (value);
