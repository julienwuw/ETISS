...
VirtualStruct::Field::Field(VirtualStruct & parent,const std::string & name,const std::string & prettyname,int flags,size_t width,size_t bitwidth) :
    parent_(parent),
    name_(name),
...
VirtualStruct::Field::Field(VirtualStruct & parent,const std::string & name,const std::string & prettyname,int flags,size_t width,bool virtual_enabled,std::function<uint64_t()> lread,std::function<void(uint64_t)> lwrite,size_t bitwidth) :
    parent_(parent),
    name_(name),
    prettyname_(prettyname),
    flags_(flags),
...
uint64_t VirtualStruct::Field::read() const
{
    ...
    case LAMBDA:
        ...
        {
            throw std::runtime_error("VirtualStruct is configured to use lambda expressions but no read function was provided.");
        }
        break;
    case PREFER_LAMBDA:
        if (lread)
...
void VirtualStruct::Field::write(uint64_t val)
{
    ...
    case LAMBDA:
        ...
        {
            //throw std::runtime_error("VirtualStruct is configured to use lambda expressions but no read function was provided."); moved to start of function
        }
        break;
    case PREFER_LAMBDA:
        if (lwrite)
...
bool VirtualStruct::Field::applyAction(const etiss::fault::Fault & f, const etiss::fault::Action & a,std::string & errormsg){
    if (!(flags_&A)){
...
bool VirtualStruct::Field::_applyBitflip(unsigned position,uint64_t fault_id){
    ...
    errval = val^errval;
    write(errval);
    etiss::log(etiss::INFO,std::string("Injected bitflip in ") +name_ + " " + etiss::toString(val) + "->" + etiss::toString(errval));
    return true;
}
bool VirtualStruct::Field::_applyAction(const etiss::fault::Fault & f, const etiss::fault::Action & a,std::string & errormsg){
    return false;
}
...
VirtualStruct::VirtualStruct(void * structure,std::function<void(Field*)> dtor) : structure_(structure),dtor_(dtor),closed(false) {
        ...
        if (*iter){
            if ((*iter)->delete_){
                ...
                if (dtor_) {
                    ...
                    etiss::log(etiss::ERROR,"The delete function passed to etiss::VirtualStruct is null. This will likely result in a memory leak.");
                }
            }
        }
    }
...
bool VirtualStruct::addField(VirtualStruct::Field * f,bool noerrorprint)
{
    ...
    {
        if (!noerrorprint)
            etiss::log(etiss::ERROR,"Failed to add an field to etiss::VirtualStruct due to empty name",f->name_,f->prettyname_,ETISS_SRCLOC);
        return false;
    }
    if (fieldNames_.find(f->name_) != fieldNames_.end())
    {
        if (!noerrorprint)
            etiss::log(etiss::ERROR,"Failed to add an field to etiss::VirtualStruct due to duplicated name",f->name_,f->prettyname_,ETISS_SRCLOC);
        return false;
    }
    if ( (!f->prettyname_.empty()) && (fieldPrettyNames_.find(f->prettyname_) != fieldPrettyNames_.end()))
    {
        if (!noerrorprint)
            etiss::log(etiss::ERROR,"Failed to add an field to etiss::VirtualStruct due to duplicated prettyname",f->name_,f->prettyname_,ETISS_SRCLOC);
        return false;
    }
...
bool VirtualStruct::acceleratedTrigger(const etiss::fault::Trigger & t,int32_t fault_id){
	if (acceleratedTrigger_){
		std::cout << "acceleratedTrigger called" << std::endl;
		return acceleratedTrigger_(t,fault_id);	
	} else {
		std::cout << "no acceleratedTrigger" << std::endl;
	}
	return false;
}
...
std::list<std::string> VirtualStruct::listFields(){
    ...
    foreachField([&ret](std::shared_ptr<Field> f){if (f) ret.push_back(f->name_);});
    return ret;
}
...
bool VirtualStruct::applyAction(const etiss::fault::Fault & fault,const etiss::fault::Action & action,std::string & errormsg){
    switch (action.getType()){
    case etiss::fault::Action::COMMAND: // handle command
    {
        if (!applyCustomAction){
            errormsg = getInjectorPath() + ": VirtualStruct::applyCustomAction function not set. cannot handle custom actions.";
            return false;
        } else {
...
void copy(VirtualStruct & dst,VirtualStruct & src,std::list<std::shared_ptr<VirtualStruct::Field> > & notPresent,std::list<std::shared_ptr<VirtualStruct::Field> > & notWriteable, std::list<std::shared_ptr<VirtualStruct::Field> > unknown,bool pretend ,std::list<std::shared_ptr<VirtualStruct::Field> > * src_private ,std::list<std::shared_ptr<VirtualStruct::Field> > * dst_private)
{
...
std::shared_ptr<etiss::fault::Injector> etiss::fault::Injector::get(const std::string & path){
    return VirtualStruct::root()->getVirtualStruct(path);
}
