...
void ValueOperationTraceGraph::filterTmp(Node * start,Node * tmp,bool hideedge,const std::string & alabels,std::ofstream & out,std::unordered_set<Node*> & nnl,std::unordered_set<std::pair<const void *,const void*> > & dependencies,std::unordered_set<Node*> & declared)
{
    ...
    else
    {
        ...
        for (auto iter = tmplinks.begin(); iter != tmplinks.end(); ++iter)
        {
            ...
            filterTmp(start,(*iter)->dst,hideedge,l,out,nnl,dependencies,declared); // hide link if this is not a direct link between a named node and the origin
            ...
            for (auto iter = nl.begin(); iter != nl.end(); ++iter)
            {
                ...
                if (cur)
                {
                    for (auto iter2 = cur->links.begin(); iter2 != cur->links.end(); ++iter2)
                    {
                        ...
                        if (curl)
                        {
                            filterTmp(cur,curl->dst,((cur == &this->start) && (nodes_.find(curl->dst->id) == nodes_.end())),curl->label,out,nnl,dependencies,defined);
                            /*
                            auto find = nodes_.find(curl->dst->id);
                            if (find != nodes_.end())
                            {
                                out << "\t" << ((size_t)curl->dst) << " [label=\"" << find->second << "\"];\n";
                            }
                            else
                            {
                            ...
                            out << "\t" << ((size_t)cur) << " -> " << ((size_t)curl->dst);
                            {
                                ...
                                if (curl->color.size() > 0)
                                    attr = attr + ((attr.size()==0)?"":" ") + "color=\"" + curl->color +"\"";
                                if (curl->label.size() > 0)
                                    attr = attr + ((attr.size()==0)?"":" ") + "label=\"" + curl->label +"\"";
                                if (attr.size() > 0)
                                    out << " [" << attr << "]";
                            }
                            out << ";\n";
...

    {
        ...
        for (auto iter = dependencies.begin(); iter != dependencies.end(); ++iter)
        {
            if (present.find(iter->first) == present.end())
            {
                ...
                std::string name = nodes_[iter->first]; // should always succeed due to filterTmp function.
                out << "\t" << ((size_t)iter->first) << " [label=\"" << name << "\"];\n";
            }
            if (present.find(iter->second) == present.end())
            {
                present.insert(iter->second);
                std::string name = nodes_[iter->second]; // should always succeed due to filterTmp function.
...
ValueOperationTraceGraphStreamer::Node* ValueOperationTraceGraphStreamer::openDestNode(const void * const dst,Node * & cleanup)
{
...
void ValueOperationTraceGraphStreamer::link(const void * dst,std::initializer_list<const void *> sources,const char * label)
{
    ...
    for (auto iter = sources.begin(); likely(iter != sources.end()); ++iter)
    {
        ...
        if (likely(ptr != 0))
        {
            srcns[pos++] = openSourceNode(ptr); // get source nodes
        }
    }
...
/*
static bool keepNode(ValueOperationTraceGraphStreamer::Node * n,const std::unordered_map<const void *,ValueOperationTraceGraphStreamer::Node*,std::hash<const void*>, std::equal_to<const void*>,etiss::ObjectPool<std::pair<const void * const,ValueOperationTraceGraphStreamer::Node*> > > & current)
{
...
void ValueOperationTraceGraphStreamer::traverse(Node * n)//,std::unordered_set<Node*> & nodes) // debug loop detection
{
    ...
    if (!path.empty())
    {
        ...
        if ((nodes_.find(path.back()->dst->id) != 0) || !keepNode(path.back()->dst,currentnodes_))  // check if cleanup possible // cleanup is possible if the originating node is a known ode or if keepNode is false
        {
...
const std::unordered_set<std::pair<const void *,const void*> > & ValueOperationTraceGraphStreamer::staticDependencies() const{
...
VariableDependencyGraph::VariableDependencyGraph(){
    ...
    deps_.rehash(1<<13); // expected behaviour is to grow at the beginning and then remain constant. bucket size should be medium large to prevent frequent rehashes at the beginning
}
...
void VariableDependencyGraph::write(std::ostream & out,const std::string & graphname,std::function<bool(const void* ,const void * ,std::string &)> filterOutCon,std::function<void(const void *,std::string & /*color*/)> nodeattr)
{
...
void VariableDependencyGraph::tf_write(const std::string & filename,const std::string & graphname,std::function<size_t(const std::string & /**<string*/,size_t /**<split pos*/> split);
...
{
    out << depth(src_?src_:dst_) << "\"" << (src_?src_->id_:"0") << "\" " << "->" /**TODO: handle undirected*/ << " \"" << (dst_?dst_->id_:"0") << "\" ";
    AttrList::print(out,"");
    out << ";\n";
}
