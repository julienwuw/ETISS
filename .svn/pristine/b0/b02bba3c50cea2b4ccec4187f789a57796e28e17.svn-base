...
int getGlobalId(){
    ...
    --cur;
    if (cur >= 0){
        std::cerr << "\n\nWARNING: etiss::error::Error ran out of ids. From now on negative ids may be assigned multiple times.\n\n" << std::endl;
        cur = -1;
    }
    return ret;
}
...
Error::Error(bool persistent,int id,const std::string & name) : persistent_(persistent),id_(id<0?getGlobalId():id),name_(name){
