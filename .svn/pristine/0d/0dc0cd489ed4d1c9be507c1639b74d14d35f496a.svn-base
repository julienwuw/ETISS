...
/**
    must return an array of zero terminated strings. the array must have a zero sentinel
    e.g.
    ...
    'L' - supports listening (VirtualStruct_setListenerCallback must be provided)
...
/**
    ...
    @attention this array MUST have the same size as the array returned by the corresponding VirtualStruct_names function
...
/**
    setter function to register the callback handle for listener supported fields. one handle per virtual struct not per field. variables with the same name in this function and the callback must have the same value
*/
