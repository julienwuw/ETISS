...
etiss::int32 MemoryDeviceMapper::iwrite(ETISS_System* sys,ETISS_CPU * cpu, etiss::uint64 addr,etiss::uint8 * buf,etiss::uint32 len)
{
	while (true){
		unsigned nlen = len;
...
etiss::int32 MemoryDeviceMapper::dread(ETISS_System* sys,ETISS_CPU * cpu, etiss::uint64 addr,etiss::uint8 * buf,etiss::uint32 len)
{
	while (true){
		unsigned nlen = len;
...
etiss::int32 MemoryDeviceMapper::dwrite(ETISS_System* sys,ETISS_CPU * cpu, etiss::uint64 addr,etiss::uint8 * buf,etiss::uint32 len)
{
	while (true){
		unsigned nlen = len;
