...
VirtualStructMemory::VirtualStructMemory(const etiss::VirtualStruct & str,std::function<uint64_t(etiss::VirtualStruct::Field *,bool & dontMount)> mountPoint,bool littleendian) : mem_(str)
{
...
bool VirtualStructMemory::write(bool debug,ETISS_CPU * cpu,etiss::uint64 addr,etiss::uint8 * buf,etiss::uint32 len)
{
    ...
    for (unsigned i = 0; i<len; ++i)
    {
        ...
        if (find != memmap_.end())
        {
            ...
            {
                std::stringstream ss;
                ss << "write to field " << f->name_ << "{" << f->prettyname_ << "}: 0x" << std::hex << f->read() << " (offset: 0x" << std::hex << find->second.second << ")" << std::endl;
