...
template<> PyObject * convert<std::string>(const std::string & s);
template<> PyObject * convert<etiss::int32>(const etiss::int32 & s);
...
template<typename T1,typename T2,typename T3> PyObject * convert(const std::tuple<T1,T2,T3> & a234)
{
...
template<typename T> PyObject * convert(const std::set<T> & set)
{
	PyObject * list = PyList_New(0);
...
template<typename T> PyObject * convert(const std::list<T> & set)
{
	PyObject * list = PyList_New(0);
	for (auto iter = set.begin(); iter != set.end(); iter++)
	{
...
template<typename K,typename V> PyObject * convert(const std::map<K,V> & set)
{
