	...
	*/
	class BlockLink {
		public:
			...
			*/
			static inline void incrRef(BlockLink*link){
				link->refcount++;
			}
			/**
			...
			*/
			...
			/**
				@brief change the value of a BlockLink pointer. handles reference count updates and cleanup of unreferenced BlockLinks
				@param link may be 0
		...
		*/
		inline BlockLink * getBlockFast(BlockLink * prev,const etiss::uint64 & instructionindex){
			if (likely(prev != 0)){
				...
				if (instructionindex >= prev->end && bl != 0 && bl->end > instructionindex){ // ->next MUST always start immediately after the current block since it is not checked here
